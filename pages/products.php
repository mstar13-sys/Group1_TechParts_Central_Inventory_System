        ...
        if (!$name||!$price||!$brand||!$catId){ $msg='Fill all required fields.'; $msgType='danger';
            ...
            try {
                if ($id>0) {
                    ...
                    // Auto-create a Stock row so the product appears in Stock Management and can be adjusted
                    $db->prepare('INSERT INTO Stock (Product_ID, Quantity, MinStock) VALUES (?,0,5)')
                       ->execute([$newProductId]);
