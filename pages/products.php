...
if ($isAdmin && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($action === 'save') {
        ...
        if (!$name||!$price||!$brand||!$catId){ $msg='Fill all required fields.'; $msgType='danger';
        } else {
            try {
                if ($id>0) {
                    ...
                    $msg='Product added.'; $msgType='success';
                }
...
<div class="card">
  <div class="table-wrap">
    <table>
      <thead>
        <tr><th>#</th><th>Name</th><th>Brand</th><th>Category</th><th>Price</th><th>Stock</th><?php if($isAdmin):?><th>Actions</th><?php endif;?></tr>
      </thead>
      <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <?php if($p['Description']):?><div style="font-size:11px;color:var(--text-muted);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?=htmlspecialchars($p['Description'])?></div><?php endif;?>
        </td>
        ...
        <td style="display:flex;gap:6px">
          <button class="btn btn-ghost btn-sm" onclick='editProduct(<?=json_encode($p)?>')'>Edit</button>
          <form method="POST">
