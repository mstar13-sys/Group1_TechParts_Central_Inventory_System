...
if ($isAdmin && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($action==='create_po') {
        ...
        if (!$supplierId || empty($productIds)) {
            ...
            } catch(Exception $e){ $db->rollBack(); error_log('[PO create] '.$e->getMessage()); $msg='Could not create Purchase Order. Please try again.'; $msgType='danger'; }
        }
        ...
        if (!$poId || !in_array($status, $allowed)) {
            $msg = 'Invalid PO or status value.'; $msgType = 'danger';
        } else {
            try {
                $db->beginTransaction();
                // Check previous status to avoid double-stocking
                $prevStatus = $db->prepare('SELECT Status FROM PurchaseOrder WHERE ID=?');
                $prevStatus->execute([$poId]);
                $prev = $prevStatus->fetchColumn();
                ...
                // Stock is updated by the DB trigger trg_add_stock_on_po_received
                // (database.sql) which fires on UPDATE PurchaseOrder when Status → Received.
                // The PHP block that previously did this was removed to prevent double-stocking.
                $db->commit();
                $msg = 'Status updated to '.$status.($status==='Received'?' — stock has been updated.':'.');
                $msgType = 'success';
            } catch (Exception $e) {
                $db->rollBack();
                error_log('[PO update_status] ' . $e->getMessage());
                $msg = 'Could not update status. Please try again.'; $msgType = 'danger';
            }
        }  // end else (valid poId + status)
    } elseif ($action==='delete_po') {
        $delId = filter_var($_POST['po_id'] ?? 0, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        if (!$delId) {
            $msg = 'Invalid PO identifier.'; $msgType = 'danger';
        } else {
            try {
                // Only Pending POs may be deleted
                $chk = $db->prepare('SELECT Status FROM PurchaseOrder WHERE ID = ?');
                $chk->execute([$delId]);
                $curStatus = $chk->fetchColumn();
                if ($curStatus !== 'Pending') {
                    $msg = 'Only Pending purchase orders can be deleted.'; $msgType = 'danger';
                } else {
                    // Status guard in SQL as well — belt-and-suspenders
                    $db->prepare('DELETE FROM PurchaseOrder WHERE ID = ? AND Status = ?')
                       ->execute([$delId, 'Pending']);
                    $msg = 'Purchase Order deleted.'; $msgType = 'warning';
                }
            } catch (PDOException $e) {
                error_log('[PO delete] ' . $e->getMessage());
                $msg = 'Could not delete Purchase Order. Please try again.'; $msgType = 'danger';
            }
        }
    }
}
    ...
    <table>
      ...
      <tr>
        ...
        <td style="display:flex;gap:6px">
          ...
          <button class="btn btn-blue btn-sm" onclick='openStatusModal(<?=json_encode($po, JSON_HEX_APOS|JSON_HEX_TAG)?>)'>Update</button>
          <?php endif;?>
...
<script>
// PHP-computed web root — ensures fetch() works in sub-folder installs too
const APP_ROOT = <?= json_encode(APP_ROOT) ?>;

// Product list for dynamically adding rows (XSS-safe: special chars hex-escaped server-side)
const products = <?=json_encode($products, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP)?>;

// Minimal HTML escape for values inserted into JS-built option labels
const esc = s => String(s).replace(/[&<>"']/g, c =>
    ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

function addPORow() {
  const opts = products.map(p =>
    `<option value="${esc(p.ID)}" data-price="${esc(p.Price)}">${esc(p.Name)} (${esc(p.Brand)})</option>`
  ).join('');
  ...
  row.innerHTML = `
    ...
    <button type="button" class="btn btn-danger btn-sm" style="margin-bottom:1px"
...
async function viewPO(id) {
  ...
  if (!intId || intId <= 0) return;

  document.getElementById('view-po-title').textContent = 'PO-' + String(intId).padStart(4, '0');
  ...
  try {
    const resp = await fetch(APP_ROOT + '/api/po_detail.php?id=' + intId, {
      credentials: 'same-origin',   // send session cookie
    });

    if (resp.status === 401 || resp.status === 403) {
      document.getElementById('view-po-body').innerHTML =
        '<p class="api-error">Session expired. Please <a href="<?= APP_ROOT ?>/login.php">log in again</a>.</p>';
      return;
    }
    if (!resp.ok) {
    ...
    document.getElementById('view-po-body').innerHTML = html;

  } catch (err) {
    console.error('[viewPO]', err);
    document.getElementById('view-po-body').innerHTML =
      '<p class="api-error">Could not load PO details. Please try again.</p>';
  }
}
</script>
