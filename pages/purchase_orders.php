...
if ($isAdmin && $_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'create_po') {
    ...
    if (!$supplierId || empty($productIds)) {
      $msg = 'Select supplier and add at least one item.';
      $msgType = 'danger';
    } else {
      try {
        $db->beginTransaction();
    ...
    if ($poId && in_array($status, $allowed)) {
      try {
        ...
        // Auto-add to stock only when transitioning TO Received
        if ($status === 'Received' && $prev !== 'Received') {
          $items = $db->prepare('SELECT Product_ID, QuantityOrdered, PurchaseOrder_ID FROM PurchaseOrderItem WHERE PurchaseOrder_ID=?');
          $items->execute([$poId]);
          $suppStmt = $db->prepare('SELECT Supplier_ID FROM PurchaseOrder WHERE ID=?');
          $suppStmt->execute([$poId]);
          $supplierId = $suppStmt->fetchColumn();
          foreach ($items->fetchAll() as $item) {
            // Update existing stock row or insert new one
            $existing = $db->prepare('SELECT ID FROM Stock WHERE Product_ID=? AND (Supplier_ID=? OR Supplier_ID IS NULL) LIMIT 1');
            $existing->execute([$item['Product_ID'], $supplierId]);
            $stockId = $existing->fetchColumn();
            if ($stockId) {
              $db->prepare('UPDATE Stock SET Quantity=Quantity+?, LastUpdated=NOW() WHERE ID=?')
                ->execute([$item['QuantityOrdered'], $stockId]);
            } else {
              $db->prepare('INSERT INTO Stock (Quantity, MinStock, Product_ID, Supplier_ID, LastUpdated) VALUES (?,5,?,?,NOW())')
                ->execute([$item['QuantityOrdered'], $item['Product_ID'], $supplierId]);
            }
          }
        }
        $db->commit();
  ...
  <div class="table-wrap">
    <table>
      ...
      <tbody>
        <?php if (empty($poList)): ?>
          <tr>
            <td colspan="9" style="text-align:center;color:var(--text-muted);padding:30px">No purchase orders found.</td>
          </tr>
        <?php endif; ?>
