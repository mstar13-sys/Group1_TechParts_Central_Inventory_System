...
if ($isAdmin && $_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($action === 'save') {
        ...
        try {
            ...
            $msg = 'Cannot delete: '.$e->getMessage(); $msgType='danger';
        }
    }
}
...
$suppliers = $db->prepare("SELECT s.*, (SELECT COUNT(*) FROM Product_has_Supplier ps WHERE ps.Supplier_ID=s.ID) AS ProductCount FROM Supplier s $where ORDER BY s.Name");
...
<div class="toolbar">
  ...
  <?php if($isAdmin):?>
  <button class="btn btn-primary" onclick="openModal('supplier-modal')">+ Add Supplier</button>
  <?php endif;?>
</div>
    ...
    <table>
      ...
      <tr>
        ...
        <td style="display:flex;gap:6px">
          <button class="btn btn-ghost btn-sm" onclick='editSupplier(<?=json_encode($s)?>')'>Edit</button>
          <form method="POST">
