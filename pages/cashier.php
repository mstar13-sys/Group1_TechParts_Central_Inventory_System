  ...
  if (empty($cart)) {
    ...
    try {
      ...
      foreach ($cart as $item) {
        ...
        if (!$product) {
          throw new Exception('A product in the cart was not found.');
        }
      ...
      if ($payMethod === 'Cash' && $tendered < $total) {
        throw new Exception('Amount tendered is less than the total.');
      }
...
<script>
  ...
  function recalc() {
    ...
    document.getElementById('cart-subtotal').textContent = '₱' + sub.toLocaleString('en-PH', {
      minimumFractionDigits: 2
    });
    document.getElementById('cart-total').textContent = '₱' + total.toLocaleString('en-PH', {
      minimumFractionDigits: 2
    });
  }
  ...
  function openCheckoutModal() {
    ...
    document.getElementById('modal-subtotal').textContent = '₱' + sub.toLocaleString('en-PH', {
      minimumFractionDigits: 2
    });
    document.getElementById('modal-discount').textContent = disc + '%';
    document.getElementById('modal-total').textContent = '₱' + total.toLocaleString('en-PH', {
      minimumFractionDigits: 2
    });
    calcChange();
  ...
  function calcChange() {
    ...
    document.getElementById('modal-change').textContent = '₱' + change.toLocaleString('en-PH', {
      minimumFractionDigits: 2
    });
  }
