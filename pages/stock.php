...
if($filter==='low')  { $where[]='s.ID IS NOT NULL AND s.Quantity<=s.MinStock AND s.Quantity>0'; }
if($filter==='empty'){ $where[]='s.ID IS NULL OR s.Quantity=0'; }
...
$alerts     = $db->query('SELECT SUM(Quantity<=MinStock AND Quantity>0) AS low_stock, SUM(Quantity=0) AS out_of_stock FROM Stock')->fetch();
