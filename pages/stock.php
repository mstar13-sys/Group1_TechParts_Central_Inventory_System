...
if($filter==='low')  { $where[]='s.Quantity<=s.MinStock AND s.Quantity>0'; }
if($filter==='empty'){ $where[]='s.Quantity=0'; }
$whereStr=implode(' AND ',$where);

$stock = $db->prepare("
    SELECT s.*, p.Name AS ProductName, p.Brand, p.Price, c.Name AS CategoryName, sup.Name AS SupplierName
    FROM Stock s
    JOIN Product p ON s.Product_ID=p.ID
    LEFT JOIN Category c ON p.Category_ID=c.ID
    LEFT JOIN Supplier sup ON s.Supplier_ID=sup.ID
    WHERE $whereStr ORDER BY s.Quantity ASC, p.Name
");
$stock->execute($params);
$stockItems = $stock->fetchAll();

$lowCount   = $db->query('SELECT COUNT(*) FROM Stock WHERE Quantity<=MinStock AND Quantity>0')->fetchColumn();
$emptyCount = $db->query('SELECT COUNT(*) FROM Stock WHERE Quantity=0')->fetchColumn();
...
<div class="card">
  <div class="table-wrap">
    <table>
      <thead>
        <tr><th>Product</th><th>Brand</th><th>Category</th><th>Supplier</th><th>Qty</th><th>Min Stock</th><th>Unit Price</th><th>Value</th><th>Last Updated</th><?php if($isAdmin):?><th>Adjust</th><?php endif;?></tr>
      </thead>
      <tbody>
      ...
      <tr>
        ...
        <td>
          <?php if($s['Quantity']==0):?>
            <span class="out-of-stock-text"><?=$s['Quantity']?></span>
          <?php elseif($s['Quantity']<=$s['MinStock']):?>
            <span class="low-stock"><?=$s['Quantity']?> ⚠</span>
          <?php else:?>
            <span style="color:var(--accent3);font-weight:600"><?=$s['Quantity']?></span>
          <?php endif;?>
        </td>
        <td style="color:var(--text-muted)"><?=$s['MinStock']?></td>
        <td>₱<?=number_format($s['Price'],2)?></td>
        <td style="color:var(--accent3)">₱<?=number_format($s['Quantity']*$s['Price'],2)?></td>
        <td style="color:var(--text-muted);font-size:12px"><?=date('M d, Y',strtotime($s['LastUpdated']))?></td>
        <?php if($isAdmin):?>
        <td>
          <button class="btn btn-ghost btn-sm" onclick='adjustStock(<?=json_encode($s)?>')'>Adjust</button>
        </td>
        <?php endif;?>
      </tr>
      <?php endforeach;?>
...
function adjustStock(s) {
  document.getElementById('adj-id').value = s.ID;
