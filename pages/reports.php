...
$voids->execute([$from,$to]); $voidCount = $voids->fetchColumn();
...
$byDay->execute([$from,$to]); $dailySales = $byDay->fetchAll();
...
");
$topProds->execute([$from,$to]); $topProducts = $topProds->fetchAll();
...
$byMethod->execute([$from,$to]); $byPayment = $byMethod->fetchAll();
...
$cashierPerf->execute([$from,$to]); $cashiers = $cashierPerf->fetchAll();
...
<div style="display:grid;grid-template-columns:2fr 1fr;gap:20px">
  ...
  <div class="card">
    ...
    <?php if(empty($dailySales)):?>
      <div class="empty-state"><div class="empty-icon">📊</div><p>No sales data for this period.</p></div>
    <?php else:?>
  ...
  <div class="card">
    ...
    <?php if(empty($byPayment)):?>
      <div class="empty-state"><div class="empty-icon">💳</div><p>No data.</p></div>
    <?php else:?>
    <?php $maxRev=max(array_column($byPayment,'Revenue')); foreach($byPayment as $pm):?>
